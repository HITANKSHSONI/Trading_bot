...
def get_selected_stock():
    """Fetch the selected stock from Flask API."""
    global selected_symbol_token, selected_quantity

    try:
        response = requests.get("http://127.0.0.1:5000/get_selected_stock")
        data = response.json()
        
        if data and data["symboltoken"] and data["quantity"]:
            selected_symbol_token = data["symboltoken"]
            selected_quantity = data["quantity"]
    except Exception as e:
        print("⚠ Error fetching stock selection:", str(e))
...
def update_live_data(live_df, new_data):
    if new_data is not None and not new_data.empty:
        if live_df is None or live_df.empty:
            ...
            # This ensures we maintain 15-minute candles in our data
...
def calculate_supertrend(df):
    ...
    try:
        ...
        if supertrend is not None:
            ...
            close_series, supertrend_series = df_copy['close'].align(df_copy["SUPERT_10_2.0"], axis=0, copy=False)
...
def main():
    ...
    while True:
        get_selected_stock()  # Update stock selection dynamically

        if selected_symbol_token is None or selected_quantity is None:
            print("⏳ Waiting for stock selection and quantity...")
            time.sleep(5)
            continue
    exchange = "NSE"
    #quantity = 1
    ...
    try:
        while True:
            ...
            # Plot updated chart every 5 iterations (to avoid too many plot windows)
