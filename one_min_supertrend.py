...
def update_live_data(live_df, new_data):
    if new_data is not None and not new_data.empty:
        if live_df is None or live_df.empty:
            ...
            # Round timestamps to the nearest 1 minute for OHLC consolidation (changed from 15 minutes)
            ...
            rounded_time = pd.Timestamp(
                ...
                day=new_time.day,
                hour=new_time.hour,
                minute=new_time.minute,  # Keep exact minute for 1-minute candles
                tz=new_time.tz
            )
            ...
            if not live_df.empty:
                ...
                last_candle_floor = last_candle_time.floor('1min')
                rounded_time_floor = rounded_time.floor('1min')
...
def main():
    ...
    print("Waiting for at least 15 minutes to collect sufficient data for Supertrend calculation...")
    ...
    try:
        while True:
            ...
            if new_data is not None and not new_data.empty:
                ...
                if not live_df.empty:
                    # Check if we have collected at least 15 candles
                    if len(live_df) >= 15:
                        if not is_strategy_active:
                            print(f"✅ Collected {len(live_df)} candles. Starting Supertrend strategy.")
                            is_strategy_active = True
                        
                        # Calculate Supertrend
                        live_df = calculate_supertrend(live_df)
                        
                        if 'SUPERT_10_2.0' in live_df.columns and is_strategy_active:
                        ...
                        remaining = max(0, 15 - elapsed_minutes)
                        print(f"Collecting data: {len(live_df)}/15 candles. ~{remaining:.1f} minutes remaining.")
            
            # Wait for next update (10-15 seconds to capture each 1-minute candle properly)
            print(f"Waiting for next update... (Current time: {datetime.datetime.now().strftime('%H:%M:%S')})")
            time.sleep(12)  # Shorter interval to capture 1-minute candles more accurately
        ...
        # Plot final chart
        if 'live_df' in locals() and not live_df.empty and len(live_df) >= 15:
            try:
                live_df = calculate_supertrend(live_df)
