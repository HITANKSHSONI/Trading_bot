...
async function handleOrderSubmit(event) {
    ...
    const selectedStockSymbol = document.getElementById('stockSearch').value;
    const selectedStockToken = selectedSymbolToken; // Token from frontend selection
    const quantity = document.getElementById('quantity').value;

    if (!selectedStockSymbol || !selectedStockToken || !quantity) {
        alert("Please select a stock and enter quantity.");
        return;
    }

    const orderData = {
        symboltoken: selectedStockToken,
        quantity: parseInt(quantity)  // Ensure quantity is an integer
    };

    console.log("Sending stock selection to Flask:", orderData);

    try {
        const response = await fetch('http://127.0.0.1:5000/start_bot', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            alert("Bot started successfully with quantity: " + quantity);
        } else {
            alert("Error starting bot: " + (result.message || "Unknown error"));
        }
    } catch (error) {
