...
stop_loss = None
take_profit = None
# Add these global variables after your existing declarations
trade_history = []
total_profit_loss = 0
        ...
        if "data" in response_data:
            ...
            if market_data:
                ...
                live_data = {
                    ...
                    "volume": int(market_data[0].get("tradeVolume", 0))
                }
...
def apply_trading_strategy(df, trading_symbol, quantity=1):
    ...
    global trade_history, total_profit_loss, current_trade_entry_price, current_trade_entry_time
    global trade_count, winning_trades, losing_trades
    ...
    if signal_changed:
        ...
        if active_position == "long":
            ...
            print(f"🔄 Closing long position at {exit_price} at {latest_candle.name}")
            print(f"{result_emoji} P&L: ₹{trade_profit:.2f} ({trade_profit_pct:.2f}%)")
            print(f"💰 Total P&L: ₹{total_profit_loss:.2f} | Trades: {trade_count} | Win/Loss: {winning_trades}/{losing_trades}")
            ...
            exit_price = latest_candle['close']
            trade_profit = (entry_price - exit_price) * quantity
            trade_profit_pct = (entry_price / exit_price - 1) * 100
            total_profit_loss += trade_profit
            trade_count += 1
            ...
            print(f"🔄 Closing short position at {exit_price} at {latest_candle.name}")
            print(f"{result_emoji} P&L: ₹{trade_profit:.2f} ({trade_profit_pct:.2f}%)")
            print(f"💰 Total P&L: ₹{total_profit_loss:.2f} | Trades: {trade_count} | Win/Loss: {winning_trades}/{losing_trades}")
        ...
        if current_signal == 'buy' and active_position is None:
            ...
            current_trade_entry_time = latest_candle.name
    ...
    # Check for stop loss or take profit hits on existing positions
        ...
        if active_position == "long":
            ...
            if latest_candle['low'] <= stop_loss:
                ...
                print(f"❌ P&L: ₹{trade_profit:.2f} ({trade_profit_pct:.2f}%)")
                print(f"💰 Total P&L: ₹{total_profit_loss:.2f} | Trades: {trade_count} | Win/Loss: {winning_trades}/{losing_trades}")
...
def save_trade_history():
    ...
    try:
        ...
        filename = f"trade_history_{datetime.datetime.now().strftime('%Y%m%d')}.csv"
